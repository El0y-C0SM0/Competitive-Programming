...
int t[TAM][TAM]; // defina inicialmente com -1
...
/// @param W capacidade restante
/// @param p vetor de pesos
/// @param v vetor de valores
int knapsack(const int n, const int W, vector<int> &p, vector<int> &v) {
    if(t[n][W] != -1) return t[n][W];
    ...
    for(int i{0}; i < n; i++) {
        for(int w{0}; w < W; w++) {
            if(!i || !w) t[i][w] = 0;
                ...
                t[i][w] = max(t[i-1][w], v[i - 1] + t[i-1][w - p[i - 1]]);
            else t[i][w] = t[i - 1][w];
        }
    }
